...
class NodeConfig:
    ...
    # NEW: Stop when loss improvement < threshold
...
class HeliosNode:
    ...
    def __init__(self, config: NodeConfig):
        ...
        logger.info(
            f"Initialized Helios node {self.node_id} on device {self.device}")
        ...
        async def health_check():
            ...
            try:
                ...
                if cpu_percent > 95 or memory_percent > 95:
                    raise HTTPException(
                        status_code=503, detail="System resources exhausted")
    ...
    async def start(self):
        ...
        try:
            ...
            self.dht_node = RealDhtNode(
                self.node_id, self.config.port, self.config.bootstrap_host, self.config.bootstrap_port)
            await self.dht_node.start()
            logger.info(
                f"Bootstrapping to {self.config.bootstrap_host}:{self.config.bootstrap_port}")

            # Start health server
            config = uvicorn.Config(
                self.app, host="0.0.0.0", port=self.config.http_port, log_level="info")
    ...
    async def _gossip_loop(self):
        ...
        while True:
            try:
                ...
                logger.debug(
                    f"Published sharded parameters for epoch {self.current_epoch}")
            except Exception as e:
    ...
    async def _training_loop(self):
        ...
        while True:
            try:
                ...
                if self.current_epoch >= self.config.max_epochs:
                    logger.info(
                        f"Reached maximum epochs ({self.config.max_epochs}), stopping training")
                    break
                ...
                if self.current_epoch == last_epoch:
                    ...
                    if repeat_count > 1:
                        logger.warning(
                            f"Epoch {self.current_epoch} is repeating {repeat_count} times!")
                else:
                    repeat_count = 0
                ...
                logger.info(
                    f"Training round {self.total_rounds}: loss={loss:.4f}, accuracy={accuracy:.4f}, peers={self.peer_count}")
            except Exception as e:
        ...
        if self.training_stats["loss"]:
            ...
            logger.info(
                f"Final loss: {final_loss:.4f}, Final accuracy: {final_accuracy:.2f}%")
    ...
    async def _train_epoch(self) -> Tuple[float, float]:
        ...
        for batch_idx in range(10):
            ...
            data = torch.randn(self.config.batch_size,
                               1, 28, 28).to(self.device)
            target = torch.randint(
                0, 10, (self.config.batch_size,)).to(self.device)
            ...
            for peer_id in peer_ids:
                try:
                    ...
                    if not meta or 'param_names' not in meta:
                        continue
                    shard_param_names = [name for idx, name in enumerate(
                        meta['param_names']) if idx % self.num_shards == self.shard_index]
                    peer_shard_params = {}
                    ...
                    if peer_shard_params:
                        ...
                        logger.debug(
                            f"Fetched sharded params from peer {peer_id} at epoch {latest_epoch}")
                except Exception as e:
                    logger.warning(
                        f"Failed to fetch sharded peer parameters: {e}")
                    continue
            if peer_params:
                await self._federated_averaging(peer_params)
                logger.info(
                    f"Aggregated sharded parameters from {self.peer_count} peers")
        except Exception as e:
    ...
    async def _federated_averaging(self, peer_params: List[Dict]):
        ...
        try:
            ...
            for key in current_params.keys():
                param_sum = torch.tensor(
                    current_params[key], dtype=torch.float32)
                for peer_param in peer_params:
                    if key in peer_param:
                        param_sum += torch.tensor(
                            peer_param[key], dtype=torch.float32)
                averaged_params[key] = (
                    param_sum / (len(peer_params) + 1)).tolist()
    ...
    def _get_model_state(self) -> Dict:
        ...
        for idx, (name, param) in enumerate(params):
            if idx % self.num_shards == self.shard_index:
                shard_params[name] = _to_serializable(
                    param.clone().detach().cpu())
        return shard_params
            ...
            for idx, (name, param) in enumerate(self.model.named_parameters()):
                if idx % self.num_shards == self.shard_index and name in state_dict:
                    ...
                    param.copy_(torch.tensor(
                        v, dtype=param.dtype, device=param.device))
    ...
    def _should_stop_early(self, loss: float, accuracy: float) -> bool:
        ...
        if accuracy >= self.config.min_accuracy:
            logger.info(
                f"Accuracy threshold reached: {accuracy:.2f}% >= {self.config.min_accuracy}%")
            return True
        ...
        if loss_improvement > self.config.convergence_threshold:
            ...
            self.patience_counter = 0
            logger.debug(
                f"Loss improved by {loss_improvement:.6f}, resetting patience")
        else:
            # No significant improvement
            self.patience_counter += 1
            logger.debug(
                f"No significant loss improvement, patience: {self.patience_counter}/{self.config.patience}")

        # Check if we've exceeded patience
        if self.patience_counter >= self.config.patience:
            logger.info(
                f"Patience exceeded: no improvement for {self.config.patience} epochs")
            return True
    ...
    def _on_peer_discovered(self, peer_info: PeerInfo):
        ...
        logger.info(
            f"Discovered peer {peer_info.node_id} at epoch {peer_info.epoch}")
        self.peer_count = self.dht_node.get_peer_count()

    def _on_parameters_received(self, peer_info: PeerInfo):
        """Callback when parameters are received from a peer."""
        logger.info(
            f"Received parameters from peer {peer_info.node_id} (loss: {peer_info.loss:.4f}, accuracy: {peer_info.accuracy:.2f}%)")
...
async def main():
    ...
    config = NodeConfig(
        ...
        bootstrap_host=os.getenv(
            "HELIOS_BOOTSTRAP_HOST", "bootstrap.jami.net"),
        ...
        convergence_threshold=float(
            os.getenv("HELIOS_CONVERGENCE_THRESHOLD", "0.001")),
